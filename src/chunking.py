...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for s in splits:
            sub_pieces = self._split(s, next_seps) if len(s) > self.chunk_size else [s]
            for p in sub_pieces:
                if not p:
                    continue
                added_len = len(p) + (len(sep) if current_group else 0)
                if current_group and current_len + added_len > self.chunk_size:
                    final_chunks.append(sep.join(current_group))
                    current_group = [p]
                    current_len = len(p)
                else:
